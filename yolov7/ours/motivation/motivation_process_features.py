...
Core question:
Do correct and erroneous annotations show different training-process dynamics?
...
def build_records(gid_to_info, gid_to_metric, k=0.2):
    ...
    for gid, metric in gid_to_metric.items():
        ...
        records.append({
            ...
            "label": "correct" if fault_type == 0 else "error",
            "conf_list": conf,
            "iou_list": iou,
...
def plot_trajectory(records, key, ylabel, output_path):
    groups = {
        ...
        "error": [r[key] for r in records if r["label"] == "error"],
    }
    if not groups["correct"] or not groups["error"]:
        return

    min_epochs = min(min(len(v) for v in values) for values in groups.values())
    plt.figure(figsize=(7, 4))
    palette = {"correct": "#2f7d32", "error": "#c62828"}
    ...
    plt.xlabel("Epoch")
    plt.ylabel(ylabel)
    plt.title(f"Training-process {ylabel}: correct vs error")
    plt.grid(alpha=0.25)
    plt.legend()
    plt.tight_layout()
...
def plot_feature_distributions(records, output_path):
    ...
    sns.boxplot(data=plot_df, x="feature", y="value", hue="label", showfliers=False)
    plt.xticks(rotation=20, ha="right")
    plt.xlabel("Process feature")
    plt.ylabel("Value")
    plt.title("Derived process features separate correct and erroneous boxes")
...
def write_summary(records, output_path):
    ...
    with open(output_path, "w", newline="") as f:
        fieldnames = [
            ...
            "correct_median",
            "error_median",
            "cliffs_delta_error_minus_correct",
        ]
        writer = csv.DictWriter(f, fieldnames=fieldnames)
        writer.writeheader()
        for feature_name in feature_names:
            correct_values = [r[feature_name] for r in correct]
            error_values = [r[feature_name] for r in error]
            writer.writerow({
                "feature": feature_name,
                "correct_mean": np.mean(correct_values) if correct_values else "",
                "error_mean": np.mean(error_values) if error_values else "",
                "correct_median": np.median(correct_values) if correct_values else "",
                "error_median": np.median(error_values) if error_values else "",
                "cliffs_delta_error_minus_correct": cliffs_delta(error_values, correct_values),
            })
...
def run(args):
    ...
    error_count = sum(1 for r in records if r["label"] == "error")
    ...
    print(f"used matched boxes={len(records)}, correct={correct_count}, error={error_count}")
    ...
    plot_feature_distributions(records, output_dir / "process_feature_distributions.png")
    ...
    for filename in [
        ...
        "process_feature_distributions.png",
...
def parse_args():
    parser = argparse.ArgumentParser(description="Visualize training-process motivation for annotation-error ranking.")
