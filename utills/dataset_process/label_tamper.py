...
def perturb_box(box, max_iou=0.5):
    ...
    for _ in range(100):
        ...
        if 0.1 < iou < max_iou:
            return new_box, True
    return clip_box_to_unit(*box), False

def generate_far_box(box, w, h):
    old_xc, old_yc, old_w, old_h = box
    """生成一个远离原目标的新框（假框）"""
    ...
    for _ in range(200):
        ...
        if compute_iou((xmin, ymin, xmax, ymax), (new_xmin, new_ymin, new_xmax, new_ymax)) <= 0.1:
            ...
            return new_xc, new_yc, new_w, new_h, True
    # fallback
    return old_xc, old_yc, old_w, old_h, False
...
def modify_labels():
    ...
    for label_file in files_to_modify:
        ...
        if len(lines) <= 1:
            # 只有一个框子就不要再删除了
            error_type = random.choice(["class", "box", "redundancy"])
        else:
        ...
        # --- box error ---
            ...
            (xmin, ymin, xmax, ymax), is_success = perturb_box((xmin, ymin, xmax, ymax), MAX_IOU_BOX_ERROR)
            if is_success:
        ...
        # --- move error (额外添加一个多余框) ---
            ...
            new_xc, new_yc, new_w, new_h, is_success = generate_far_box(
            ...
            else: # 如果 redundancy 没成功 就改class error
                new_class = class_idx
                while new_class == class_idx:
                    new_class = random.randint(0, NUM_CLASSES - 1)
                lines[line_idx] = f"{new_class} {x_center:.6f} {y_center:.6f} {w:.6f} {h:.6f}"
